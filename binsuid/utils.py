...
# Linux capability bit order (capability.h) — used when capsh is unavailable.
...
def decode_cap_eff_hex(mask: str) -> list[str]:
    """Decode /proc/pid/status CapEff hex mask into CAP_* tokens."""
    cleaned = mask.strip().lower()
    ...
    try:
        ...
        return []
    caps: list[str] = []
    for bit, name in enumerate(_LINUX_CAPABILITY_NAMES):
        if value & (1 << bit):
            caps.append(name)
    return caps
